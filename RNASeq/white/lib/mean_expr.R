...
namehead = "K562.white.RNAseq"
nametail = ".genes.results"

# Set the replicates number in this cycle:
for (num in 1:2) {
...
write.table(as.data.frame(alldata), file="K562.white.hg38.TPM", row.names=F, col.names=T, sep="\t", quote=F)
write.table(as.data.frame(FPdata), file="K562.white.hg38.FPKM", row.names=F, col.names=T, sep="\t", quote=F)
write.table(as.data.frame(finalmean), file="K562.white.hg38.mean.TPM", row.names=F, col.names=T, sep="\t", quote=F)
write.table(as.data.frame(finalPKmean), file="K562.white.hg38.mean.FPKM", row.names=F, col.names=T, sep="\t", quote=F)
